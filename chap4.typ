...
从而有
$
  u(alpha, beta, gamma) = e^(- i/2 vb(sigma) dot vb(x)) = e^(mat(-i/2 x_3, -i/2(x_1 - i x_2); -i/2(x_1 + i x_2), i/2 x_3))
$
...
转动可以用一个旋转算符$hat(R)(vb(theta))$来表示，其中$vb(theta)$是一个三维空间的旋转矢量。对于一个粒子的位置算符$hat(r)$，旋转变换后的位置算符为

$
  hat(R)(vb(theta)) vb(r) = R(vb(theta)) vb(r)
  hat(R)(vb(theta)) psi(vb(r)) = psi'(vb(r)) = psi(R^(-1)(vb(theta)) vb(r))
$
考虑无穷小变换$delta vb(theta)$
$
  R(delta vb(theta)) = 1 - i delta vb(theta) dot hat(vb(J))
$
其中$hat(vb(J))$是角动量算符，是转动的生成元，旋转算符可以表示为
$
  hat(R)(vb(theta)) = e^(- i vb(theta) dot hat(vb(J)))
$
其中$hat(vb(J))$是角动量算符的坐标表象。转动不变性意味着哈密顿量$hat(H)$与旋转算符$hat(R)(vb(theta))$对易，即$[hat(H), hat(R)(vb(theta))] = 0$，从而$hat(H)$与角动量算符$hat(vb(J))$对易，即$[hat(H), hat(vb(J))] = 0$，这表明角动量是一个守恒量，即角动量守恒定律。

== 角动量

=== 角动量的量子化

角动量是矢量$vb(J)$，其分量满足$"su"(2)$代数。角动量大小与量子数的关系为
$
  abs(vb(J)) = hbar sqrt(j(j+1))
$
其能级是分立的，角动量也是分立的，$j$只能取整数和半整数。角动量的投影也有特定的取值
$
  J_z = hbar m, m = -j, -j+1, ..., j-1, j
$
#newpara()
两个角动量的叠加
$
...
$
  ...
  J = & 3/2  &    & 3/2
$
$e,nu$之间的组合，$L=0$；$e,nu$总的J，可以是0或1；$N(Z+1)$与$e,nu$系统之间的$L$必须是1或2

*CG系数*
$
  ket(j_1 m_1) ket(j_2 m_2) = sum_(j=abs(j_1 - j_2))^(j_1 + j_2) C_(m m_1 m_2)^(j j_1 j_2) ket(j m)
$
...
- 事实上，介子、重子排列上的一个横行都是一组同位旋多重态
  #figure(
    image("pic/builds/1.6.pdf", width: 80%),
    numbering: none,
  )
  #figure(
    image("pic/builds/1.7.pdf", width: 80%),
    numbering: none,
  )
...
- $pi^+ + p -> pi^+ + p$
  - 初态同位旋：$ket(1\,1) ket(1/2\,1/2) = ket(3/2\,3/2)$
  - 末态同位旋：$ket(1\,1) ket(1/2\,1/2) = ket(3/2\,3/2)$
  - 反应振幅：$braket(3/2\, 3/2, 3/2\, 3/2) = M_3$，概率为$abs(M_3)^2$
- $pi^- + p -> pi^- + p$
  - 初态同位旋：$ket(1\,-1) ket(1/2\,1/2) = sqrt(1/3) ket(3/2\, -1/2) - sqrt(2/3) ket(1/2\, -1/2)$
  - 末态同位旋：$ket(1\,-1) ket(1/2\,1/2) = sqrt(1/3) ket(3/2\, -1/2) - sqrt(2/3) ket(1/2\, -1/2)$
  - 反应振幅有两部分
    - $I = 3/2$部分：$sqrt(1/3) sqrt(1/3) braket(3/2\, -1/2, 3/2\, -1/2) = 1/3 M_3$
    - $I = 1/2$部分：$sqrt(2/3) sqrt(2/3) braket(1/2\, -1/2, 1/2\, -1/2) = 2/3 M_1$
  - 总振幅：$1/3 M_3 + 2/3 M_1$，概率为$abs(1/3 M_3 + 2/3 M_1)^2$
- $pi^- + p -> pi^0 + n$
  - 初态同位旋：$ket(1\,-1) ket(1/2\,1/2) = sqrt(1/3) ket(3/2\, -1/2) - sqrt(2/3) ket(1/2\, -1/2)$
  - 末态同位旋：$ket(1\,0) ket(1/2\,-1/2) = sqrt(2/3) ket(3/2\, -1/2) + sqrt(1/3) ket(1/2\, -1/2)$
  - 反应振幅有两部分
    - $I = 3/2$部分：$sqrt(1/3) sqrt(2/3) braket(3/2\, -1/2, 3/2\, -1/2) = sqrt(2)/3 M_3$
    - $I = 1/2$部分：$sqrt(2/3) sqrt(1/3) braket(1/2\, -1/2, 1/2\, -1/2) = sqrt(2)/3 M_1$
  - 总振幅：$sqrt(2)/3 M_3 - sqrt(2)/3 M_1$，概率为$abs(sqrt(2)/3 M_3 - sqrt(2)/3 M_1)^2$
- 比较过程$pi^+,pi^-$的散射概率
  - $pi^+ + p -> pi^+ + p$概率为$abs(M_3)^2$
  - $pi^- + p -> pi^- + p$概率为$abs(1/3 M_3 + 2/3 M_1)^2$
  - $pi^- + p -> pi^0 + n$概率为$abs(sqrt(2)/3 M_3 - sqrt(2)/3 M_1)^2$
  - 如果$p p$散射的中间态是一个同位旋$3/2$的粒子$(Delta)$，那么只有$M_3$参与这个过程，$M_1=0$；散射概率$sigma_1 : (sigma_2 + sigma_3)$将是$3:1$
...
- $C$宇称是电荷共轭变换的宇称：粒子→反粒子
  - C变换使$e^+ -> e^-$
  - C变换使$pi^0 -> + pi^0 或 - pi^0$
- $hat(C)^2 = 1$，因此$C$宇称的本征值只能是$+1$或$-1$
- 如果是中性粒子，而且反粒子是自身
- $C$一个可相乘的量子数
  - 内禀C宇称
    - 光子的C宇称为$-1$，电磁场由移动的电荷引起，在C变换下应该反号，$C_gamma=-1$
  - 系统的C宇称，各子体的乘积
    - $pi^0$的C宇称，各子体的乘积
      $
        pi^0 -> gamma gamma\
        C_(pi^0) = C_gamma C_gamma = 1
      $
