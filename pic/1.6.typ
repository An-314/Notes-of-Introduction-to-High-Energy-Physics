...
#let octet(
  ...
  cetz.canvas(length: scale, {
    ...
    line((-2, 0.0), (2, 0.0), stroke: dashed)
    line((-2, -y1), (2, -y1), stroke: dashed)
    ...
    content((-1.10, -1.7), [$Q = -1$])
    content((0.10, -1.7), [$Q = 0$])
    content((1.55, -1.7), [$Q = +1$])
  })
}
...
#let octet-quark(
  ...
  cetz.canvas(length: scale, {
    ...
    // 节点
    meson_node(-x1, 0.0, [$macron(u) d$], outer_r)
    meson_node(-xh, y1, [$d macron(s)$], outer_r)
    meson_node(xh, y1, [$u macron(s)$], outer_r)
    meson_node(x1, 0.0, [$u macron(d)$], outer_r)
    meson_node(xh, -y1, [$macron(d) s$], outer_r)
    meson_node(-xh, -y1, [$d macron(u)$], outer_r)

    meson_node(C1.at(0), C1.at(1), [$u macron(u) + d macron(d)$], inner_r)
    meson_node(C2.at(0), C2.at(1), [$u macron(u) + d macron(d) - 2 s macron(s)$], inner_r)
...
// 用法
#stack(dir: ltr)[
  #octet(scale: 1.3cm, outer_r: 10pt, inner_r: 10pt)][
  #octet-quark(scale: 1.3cm, outer_r: 10pt, inner_r: 10pt)
]
